...
from .llm_wealth_mgmt import WealthMgmtLlmExtractor, load_llm_config_from_env
    ...
    def _merge_rule_and_llm(rule_recs: list[PurchaseRecord], llm_recs: list[PurchaseRecord]) -> list[PurchaseRecord]:
        ...
        for lr in llm_recs:
            ...
            if (k, name, typ) in seen:
                continue
            if k and k in idx_by_amount:
                # 有相同金额，尝试补齐已有记录
                merged_indices = idx_by_amount[k]
                best_idx = merged_indices[0]
                merged[best_idx] = _dedupe_records([merged[best_idx], lr])[0]
            else:
                merged.append(lr)
            seen.add((k, name, typ))
        return merged
    ...
    def process_one(ann: Announcement, *, keyword: str, attempt: int) -> tuple[str, list[PurchaseRecord], bool]:
        ...
        try:
            if log_announcements:
                logger.info(
                    "公告链接 id=%s detail=%s pdf=%s",
                    ann.announcement_id,
                    ann.detail_url,
                    ann.pdf_url,
                )
            pdf_bytes = download_pdf_bytes(get_client(), ann.pdf_url)
            pdf_path = pdf_dir / f"{ann.announcement_id}.pdf"
            pdf_path.write_bytes(pdf_bytes)
            tables, text, text_source = parse_pdf(pdf_path, max_pages=max_pdf_pages)

            rule_recs = extract_records(
                tables=tables,
                text=text,
            ...
            if llm_extractor is not None:
                if prefilter_enabled:
                    llm_relevant = llm_extractor.prefilter(
                        ann=ann,
                        pdf_text=text,
                        tables=tables,
                    )
                    logger.info("LLM预筛完成 attempt=%s id=%s relevant=%s", attempt, ann.announcement_id, llm_relevant)
                    if not llm_relevant:
                        logger.info("LLM筛选为非理财公告 id=%s", ann.announcement_id)
                        return ann.announcement_id, [], False

                if llm_mode == "always" or (llm_mode == "fallback" and not rule_recs):
                    llm_recs = llm_extractor.extract(
                        ann=ann,
                        pdf_text=text,
                        tables=tables,
                        fallback_to_title=_clean_title(ann.title),
                        text_source=text_source,
                    )

            recs = _merge_rule_and_llm(rule_recs, llm_recs)
            if recs:
            ...
            logger.warning("处理失败 id=%s err=%s", ann.announcement_id, str(exc)[:200])
            raise
